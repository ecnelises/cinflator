...
####C 字符串的缺陷####
说了这么多，稍微总结一下。都说 C 语言的字符串是一大坑，那到底坑在哪些地方呢？其实，话说回来，所谓 C 字符串里出现的很多缺陷，根源都是 C 语言本身动态能力的孱弱。有不少的语言对字符串的处理也是“用完就丢”式的，就像 C 的``char *``一样。但是借助封装或者语言本身的特性，这些语言可以轻易地实现字符串的输入输出而不必挂念内存问题。